...
class LightPulse():
    ...
    def FrequencyScan(self,number):
        ...
        # make a time period 1 second long
        print Period*self.OutputSamples
        t = np.linspace(0.,Period,Period*self.OutputSamples)
        
        # make a Voltage waveform with ____ log-spaced frequencies in the range from the before and after offsets. Amplitude is 0.5. Length of the waveform is 1s
        V = np.append(V,make_log_noise(self.Offset_Before,self.Offset_After,Num_freqs,Amplitude_frac*self.A,t))
        T = np.append(T, t+t0)
        #plt.plot(T,V)
        #plt.show()
...
# 
#                     V = np.append(V,0.05*self.A*np.sin(2*np.pi*f*t))
#                     T = np.append(T, t+t0)
#                     t0+= t[-1]   
#==============================================================================
...
class OutPutData():
    ...
    def Save_data2(self,Data,save_location,save_filename,index):
        #Function to save the data if you are hardcoding the filename and location.
        #index is an integer that can be added to the hardcoded filename
        
        save_extension = '.Raw Data.dat' 
        
        self.SaveName = save_filename
        self.Path = save_location + save_filename + str(index) + save_extension        
        
        Variables = 'Time (s)\tGeneration (V)\tPC (V)\tPL (V)'
        
        numpy.savetxt(self.Path,Data,delimiter='\t',header=Variables)
    ...
    def Save_Inf2(self,List,save_location,save_filename,index):
        #Function to save the inf file if you are hardcoding the filename and location.
        #index is an integer that can be added to the hardcoded filename
        ...
        with open(save_location + save_filename + str(index) + '.inf','w') as text_file:
            text_file.write(s)
...
class Test(gui.MyFrame1,OutPutData):
    ...
    def Save(self,event,index,save_folder,save_name):
        ...
        self.Save_data2(self.Data,save_folder,save_name,index)        
        
        # print info locals
        self.Save_Inf2(self.Make_List_For_Inf_Save(event),save_folder,save_name,index)
    ...
    def GetValues_shortLS(self,event):
        #Hardcoded values to describe the short light soak waveform        
        
        
        # CHANGE THESE
        self.Intensity = 0.3 #V
        self.Peroid = 1.0 #s        
        
        # DON'T CHANGE THESE
        self.Waveform = 'Square'
        self.Offset_Before = 1.0 #ms
        self.Offset_After = 1.0 #ms
        self.Threshold = 0.0 #mA
        self.Binning = self.CHK_int(self.m_Binning,event)
        self.Averaging = self.CHK_int(self.m_Averaging,event)
        self.Channel = self.m_Output.GetStringSelection()
    
    def GetValues_longLS(self,event):
        #Hardcoded values to describe the long light soak waveform        
    ...
    def Perform_Standard_Measurement(self,event):
        global DAQmx_InputSampleRate, DAQmx_OutPutSampleRate
        #THIS is what happens when the go button is pressed        
        
        save_folder = 'D:/Mal/H101/H66/Initial/CX/'
        save_name = 'H66-CX-1'          
        
        
        cumul_time = 0.0
        
        num_short_light_soaks = 10 # how many times to repeat the short LS -> measure QSSPL cycle
        num_long_light_soaks = 59 # how many times to repeat the long LS -> measure QSSPL cycle
        
        #SHORT LIGHT SOAK LOOP        
        for i in range(1,num_short_light_soaks+1):        
            print '--Short light soak. Iteration number: ' + str(i)
            
            #do the light soak using a reduced sample rate
            DAQmx_InputSampleRate = float64(1.2e3) #max is float64(1e6), well its 1.25MS/s/channel 
            DAQmx_OutPutSampleRate = float64(1.2e3)
            self.measurement_type = 'shortLS'
            self.Perform_Measurement(event)
            #self.PlotData()
            cumul_time = cumul_time + self.Peroid  
            short_soak_period = self.Peroid
            self.Save(event,i,save_folder,save_name + '-S-light-soak-' + str(cumul_time) + '-')            
            
            print '--short Light soak complete. cumulative light soak time so far:' + str(cumul_time)            
            
            
            #do the standard measurement with max sample rate
            DAQmx_InputSampleRate = float64(1.2e6) #max is float64(1e6), well its 1.25MS/s/channel 
            DAQmx_OutPutSampleRate = float64(1.2e6)
            ...
            #save the standard measurement data
            self.Save(event,i,save_folder,save_name + '-measure-' + str(short_soak_period) + '-' )
            print '--Saved'
        
        #LONG LIGHT SOAK LOOP
        for j in range(i+1,i+1+num_long_light_soaks):        
            print '--Long light soak. Iteration number: ' + str(j)
            
            #do the light soak using a reduced sample rate
            DAQmx_InputSampleRate = float64(1.2e3) #max is float64(1e6), well its 1.25MS/s/channel 
            DAQmx_OutPutSampleRate = float64(1.2e3)
            self.measurement_type = 'longLS'
            self.Perform_Measurement(event)
            cumul_time = cumul_time + self.Peroid             
            
            self.Save(event,j,save_folder,save_name + '-L-lightSoak-Pt1-' + str(cumul_time) + '-')            
            
            self.Perform_Measurement(event)
            cumul_time = cumul_time + self.Peroid               
            long_soak_period = self.Peroid*2
            self.Save(event,j,save_folder,save_name + '-L-lightSoak-Pt2-' + str(cumul_time) + '-') 
            
            print '--long Light soak complete. cumulative light soak time so far:' + str(cumul_time)       
            ...
            #save the standard measurement data
            self.Save(event,j,save_folder,save_name + '-measure-' + str(long_soak_period) + '-')
            print '--Saved'
